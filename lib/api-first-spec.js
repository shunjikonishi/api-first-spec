...
function Rule(name, value, config) {
  this.name = name;
  this.validate = config.validate(value);
}
...
function Param(name, dataType, isArray, rules) {
  ...
  function validate(value) {
    if (value) {
      ...
      if (value) {
        assert.ok(validateType(value), 
          name + " must be " + (isArray ? "array of " : "") + dataType);
      }
    }
    self.rules.forEach(function(rule) {
      assert.ok(rule.validate(value), 
        "Response data " + name + " is not satisfied with " + rule.name + " rule.");
    });
  }
  var 
    self = this,
    ...
    Object.keys(rules).map(function(key) {
      if (key !== "format") {
        it("rule." + key, function() {
          var value = rules[key];
          testRule(key, value);
          temp.push(new Rule(key, value, Rules[key]));
        });
      }
    });
    self.rules = temp;
