...
console.log("validate", name, value, self.rules);
    var 
      dataNames = null,
      target = value;
    if (value) {
...
console.log("rule", rule.name, target);
      assert.ok(rule.validate(target), 
        target + ": " + rule.message(name, target));
    });
    if (children && dataNames) {
      ...
      if (strict) {
        assert.ok(dataNames === 0, "Unknown keys: " + dataNames);
      }
    }
  }
  function addRules(v) {
...
function API(config) {
  function buildParamNames(param) {
