...
function HttpClient(host, ssl) {
  ...
  function badRequestAll(optionParams) {
    function doIt(desc, data) {
      it(desc, function(done) {
        copy().params(data).badRequest(done);
      });
    }
    function processOptionValue(desc, key, value) {
      var 
        data = extend({}, params),
        ...
        if (optionValue) {
          if (util.isArray(optionValue)) {
            optionValue.forEach(function(v) {
              processOptionValue(fullName + " - " + v, fullName, v);
            });
          } else {
            processOptionValue(fullName + " - " + optionValue, fullName, optionValue);
          }
        }
